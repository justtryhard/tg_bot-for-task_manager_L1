...
def tasks(auth):
    ...
    try:
        ...
        if "Страница авторизации" not in driver.page_source:
            ...
            time.sleep(1)
            try:
                driver.get("https://justtryhard.pythonanywhere.com/work/")
        ...
        list_tasks = []
        for item in sel.xpath("//div[contains(@class, 'alert alert-warning')]"):
            list_tasks.append({
                'Адрес': item.css('h3::text').get(),
                'ID': item.css('h6::text').get(),
                'Тип': item.css('p::text').get(),

            })

        print('данные получены /', list_tasks)
